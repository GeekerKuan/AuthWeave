...
/** Selects a stored identity immediately before the network connection starts. */
public final class IdentityController
{
    ...
    /** Applies an address assignment without authentication or disk I/O on repeated connections. */
    public static synchronized void selectFor(ServerData server)
    {
        captureLauncher();
        String key = server.ip == null ? "" : server.ip.strip().toLowerCase(java.util.Locale.ROOT);
    ...
    private static void captureLauncher()
    {
        ...
        launcher = new Snapshot(
            game.getUser(), game.services(), bridge.getProfileFuture(),
            bridge.getUserApiService(), bridge.getProfileKeyPairManager()
        );
    }

    private static void restoreLauncher()
    {
        install(launcher.user, launcher.services, launcher.profileFuture,
    ...
    private static void applyOffline(LoginProfile profile)
    {
        ...
        if (name.isEmpty()) {
            throw new LoginProfileException("serverloginprofiles.error.offline_name");
        }
        UUID id = UUID.nameUUIDFromBytes(("OfflinePlayer:" + name).getBytes(StandardCharsets.UTF_8));
        User user = new User(name, id, "offline", Optional.empty(), Optional.empty());
        CompletableFuture<ProfileResult> profileFuture = CompletableFuture.completedFuture(null);
        install(user, launcher.services, profileFuture, UserApiService.OFFLINE,
    ...
    private static void applyYggdrasil(LoginProfile profile)
    {
        ...
        User user = new User(profile.playerName, id, profile.accessToken, Optional.empty(),
            Optional.ofNullable(profile.clientToken).filter(token -> !token.isBlank()));
        MinecraftSessionService session = new ThirdPartySessionService(profile.yggdrasilRoot);
        Services services = new Services(
            session, launcher.services.servicesKeySet(), launcher.services.profileRepository(),
            launcher.services.nameToIdCache(), launcher.services.profileResolver()
        );
        ...
        install(user, services, profileFuture, UserApiService.OFFLINE,
            ProfileKeyPairManager.EMPTY_KEY_MANAGER);
    }
    ...
    )
    {
        ...
        bridge.setUser(user);
        bridge.setProfileFuture(profileFuture);
        bridge.setUserApiService(userApi);
        bridge.setProfileKeyPairManager(keyManager);
        ActiveSessionService.select(services.sessionService());
    }
    ...
    private static UUID parseUuid(String raw)
    {
        ...
        if (value.length() != 32) {
            throw new LoginProfileException("serverloginprofiles.error.profile_id");
        }
